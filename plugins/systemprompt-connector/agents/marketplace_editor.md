...
- **create_skill**: Create a new skill (requires: name, description, content; optional: tags, base_skill_id). Skills are automatically added to the user's plugin.
- **update_skill**: Update an existing skill by skill_id slug (partial updates supported — only provided fields change)
- **delete_skill**: Delete a user skill by skill_id slug
...
All tools use **skill_id slugs** (e.g. 'odoo-customer-report', 'my-first-skill') — NOT UUIDs.
Use list_skills or list_agents to find the correct slug identifiers.
